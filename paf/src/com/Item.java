...
public class Item {
	...
	public String readItems(){
		...
		try{
			...
			output = "<table border='1' class='table'><tr><th scope='col'>Item Code</th>"
			...
			while (rs.next()){
				...
				output += "<td scope='row'>" + itemName + "</td>";
				output += "<td scope='row'>" + itemPrice + "</td>"; 
				output += "<td scope='row'>" + itemDesc + "</td>";
				// buttons
				output += "<td scope='row'><input name='btnUpdate' "
						+ " type='button' value='Update' class='btn btn-primary'></td>"
						+ "<td scope='row'><form method='post' action='items.jsp'>"
